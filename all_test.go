...
func (t *Arith) Add(args json.RawMessage, reply *json.RawMessage) error {
	v, err := fastjson.Parse(string(args))
	if err != nil {
		return err
	}
	a, _ := v.Get("A").Int()
	...
	*reply = append(*reply, strconv.Itoa(a+b)...)
	*reply = append(*reply, '}')
	return nil
}

func (t *Arith) Mul(args json.RawMessage, reply *json.RawMessage) error {
	v, _ := fastjson.Parse(string(args))
	a, _ := v.Get("A").Int()
	b, _ := v.Get("B").Int()
...
func (t *Arith) Div(args json.RawMessage, reply *json.RawMessage) error {
	v, _ := fastjson.Parse(string(args))
	a, _ := v.Get("A").Int()
	b, _ := v.Get("B").Int()
...
func (t *Arith) Error(args json.RawMessage, reply *json.RawMessage) error {
	panic("ERROR")
}
...
func TestServer(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		fmt.Fprintf(cli, `{"method": "Arith.Add", "id": "\u%04d", "params": [{"A": %d, "B": %d}]}
`, i, i, i+1)
		var resp ArithAddResp
		err := dec.Decode(&resp)
		if err != nil {
